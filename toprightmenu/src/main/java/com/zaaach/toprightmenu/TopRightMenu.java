...
public class TopRightMenu {
    ...
    public TopRightMenu(Activity context, int menuBackgroundRes, int itemLayoutId) {
        this.mContext = context;
        init(menuBackgroundRes, itemLayoutId);
    }

    private void init(int menuBackgroundRes, int itemLayoutId) {
        ...
        if (menuBackgroundRes != -1)
            mRecyclerView.setBackgroundResource(menuBackgroundRes);
        mRecyclerView.setLayoutManager(new LinearLayoutManager(mContext, LinearLayoutManager.VERTICAL, false));
        mRecyclerView.setOverScrollMode(View.OVER_SCROLL_NEVER);

        menuItemList = new ArrayList<>();
        mAdapter = new TRMenuAdapter(mContext, this, menuItemList,
                showIcon, showRightIcon, itemLayoutId);
    }
    ...
    private PopupWindow getPopupWindow() {
        ...
        mPopupWindow.setOnDismissListener(new PopupWindow.OnDismissListener() {
            ...
            public void onDismiss() {
                ...
                if (mMenuDismissListener != null) mMenuDismissListener.onDismiss();
            }
        });

        mAdapter.setData(menuItemList);
        mAdapter.setShowIcon(showIcon);
        mAdapter.setShowRight(showRightIcon, rightShowPosition);
        mRecyclerView.setAdapter(mAdapter);
        return mPopupWindow;
    }

    public void setMenuDismissListener(MenuDismissListener menuDismissListener) {
        mMenuDismissListener = menuDismissListener;
    }
     ...
     * @return
     */
    public TopRightMenu showRightIcon(boolean show) {
        this.showRightIcon = show;
        return this;
    }

    public void setRightShowPosition(int position) {
        rightShowPosition = position;
        mAdapter.setShowRight(showRightIcon, position);
    }
    ...
    public void showPoint(int position) {
        menuItemList.get(position).setHasPoint(true);
        mAdapter.notifyItemChanged(position);
    }

    public void hidePoint(int position) {
        menuItemList.get(position).setHasPoint(false);
        mAdapter.notifyItemChanged(position);
    }
