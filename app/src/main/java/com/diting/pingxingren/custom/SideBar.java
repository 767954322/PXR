...
public class SideBar extends View {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        //导航栏居中显示，上下各有80dp的边距
        mHeight = (int) (h - ScreenUtil.dip2px(mContext, TOP_MARGIN * 2));
        mWidth = w;
    ...
    protected void onDraw(Canvas canvas) {
        for (int i = 0; i < indexStr.length(); i++) {
            ...
            if(textTag.equals(focusTag)){
                //mPaint.setColor(Color.GREEN);
                mPaint.setColor(Color.parseColor("#00c4c2"));
            }else {
                mPaint.setColor(Color.GRAY);
            }
            canvas.drawText(textTag, xPos, singleHeight * (i + 1) + ScreenUtil.dip2px(mContext, TOP_MARGIN), mPaint);
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                int position = (int) ((event.getY() - getTop() - ScreenUtil.dip2px(mContext, 80)) / mHeight * indexStr.toCharArray().length);
                if (position >= 0 && position < indexStr.length()) {
                    setFocusTag(indexStr.substring(position,position+1));
